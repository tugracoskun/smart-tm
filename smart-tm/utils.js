...
const Utils = {
    ...
    extractPlayerData(row) {
        ...
        // 4. ADIM: Oyuncu Resmini Çek (Lazy-load uyumlu)
        let imageUrl = null;
        const imgEl = row.querySelector('img.bilderrahmen-fixed') || 
                      row.querySelector('img[src*="/spieler/"]') || 
                      row.querySelector('img[src*="/portraits/"]') || 
                      row.querySelector('td img');
        if (imgEl) {
        ...
        return {
            ...
            imageUrl: imageUrl, // Resim adresi artık eklendi
    ...
    getSofaScoreSearchUrl(name) { return `https://www.google.com/search?q=${encodeURIComponent(name + ' sofascore')}`; },
    getMackolikSearchUrl(name) { return `https://www.mackolik.com/arama/${encodeURIComponent(name)}`; },
