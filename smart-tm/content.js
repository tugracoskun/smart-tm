...
(async function () {
    ...
    async function init() {
        ...
        try {
            ...
            // Auto-detect watchlist URL from header dropdown (Runs on any page when user is logged in)
            try {
                const wlLink = document.querySelector('a[href*="/spielerWatchlist/"]');
                if (wlLink) {
                    const wlUrl = new URL(wlLink.getAttribute('href'), window.location.origin).href;
                    if (wlUrl && settings.watchlistUrl !== wlUrl) {
                        settings.watchlistUrl = wlUrl;
                        if (typeof StorageManager !== 'undefined' && typeof StorageManager.updateSettings === 'function') {
                            await StorageManager.updateSettings({ watchlistUrl: wlUrl });
                            console.log('[Smart-TM] Auto-detected watchlist URL:', wlUrl);
                        }
                    }
                }
            } catch (e) {
                console.error('[Smart-TM] Error auto-detecting watchlist URL:', e);
            }

            // Scrape notifications on load (Runs on any page)
            try {
                scrapeHeaderNotifications();
            } catch (e) {
                console.error('[Smart-TM] Error scraping notifications on load:', e);
            }

            // Auto-sync watchlist if URL is known (Runs on any page, max once per hour)
            try {
                if (settings.watchlistUrl) {
                    chrome.storage.local.get(['smarttm_watchlist_last_synced'], async (res) => {
                        const lastSynced = res.smarttm_watchlist_last_synced || 0;
                        const now = Date.now();
                        if (now - lastSynced > 60 * 60 * 1000) {
                            console.log('[Smart-TM] Watchlist has not been synced in the last hour. Starting auto-sync...');
                            const syncResult = await syncWatchlistFromUrl(settings.watchlistUrl);
                            if (syncResult.success) {
                                chrome.storage.local.set({ smarttm_watchlist_last_synced: now });
                            }
                        }
                    });
                }
            } catch (e) {
                console.error('[Smart-TM] Error checking auto-sync:', e);
            }

            // Periodic task to scrape notifications every 5 seconds (Runs on any page)
            const globalInterval = setInterval(() => {
                ...
                try {
                    ...
                    console.error('[Smart-TM] Error in global interval:', e);
                }
            }, 5000);
    ...
    function createScoutButtonsContainer(playerName) {
        ...
        // SofaScore button
        if (settings.scoutSofascore !== false) { // Default to true if undefined
            const ssBtn = Utils.createElement('a', {
                className: 'smarttm-scout-btn smarttm-sofascore',
                href: Utils.getSofaScoreSearchUrl(playerName),
                target: '_blank',
                title: 'SofaScore\'da ara'
            });
            ssBtn.innerHTML = `<img src="https://play-lh.googleusercontent.com/-ugB_WCn3_i63xmKBrKHmmxKst2oZbiZbHFvjECGeRyi58aCsRZ08whCaWBUOk34M_dS" alt="SofaScore" style="width: 100%; height: 100%; object-fit: cover; border-radius: 4px;">`;
    ...
    /**
     * Eşitleme URL'sinden izleme listesini çeker ve yerel depolama ile birleştirir.
     */
    async function syncWatchlistFromUrl(url) {
        if (!url) return { success: false, message: 'Eşitleme adresi bulunamadı.' };

        try {
            console.log('[Smart-TM] Fetching watchlist page for sync:', url);
            ...
            const rows = doc.querySelectorAll('.items > tbody > tr:not(.spacer), .responsive-table > table > tbody > tr:not(.spacer), #yw1 > table > tbody > tr:not(.spacer)');
            
            if (rows.length === 0) {
                return { success: false, message: 'İzleme listesinde oyuncu bulunamadı.' };
            }
                ...
                try {
                    ...
                    if (playerData && playerData.id && playerData.name) {
                        if (!playerData.imageUrl) {
                            const imgEl = row.querySelector('img.bilderrahmen-fixed') || row.querySelector('img');
                            if (imgEl) {
                                const src = imgEl.getAttribute('data-src') || imgEl.getAttribute('src');
                                if (src && !src.includes('placeholder') && !src.includes('blank.jpg') && !src.includes('default')) {
                                    playerData.imageUrl = src;
                                }
                            }
                        }
                        scrapedPlayers.push(playerData);
                    }
                } catch (e) {
                    console.error('[Smart-TM] Watchlist player row extraction failed:', e);
                }
            });

            if (scrapedPlayers.length > 0) {
                // Get current watchlist and merge
                if (typeof StorageManager !== 'undefined' && typeof StorageManager.getWatchlist === 'function') {
                    const currentWL = await StorageManager.getWatchlist();
                    const mergedWL = [...currentWL];
                    let addedCount = 0;

                    scrapedPlayers.forEach(scraped => {
                        const exists = mergedWL.some(p => p.id === scraped.id);
                        if (!exists) {
                            scraped.addedAt = new Date().toISOString();
                            mergedWL.push(scraped);
                            addedCount++;
                        }
                    });

                    await StorageManager.set(StorageManager.KEYS.WATCHLIST, mergedWL);
                    return { success: true, count: scrapedPlayers.length, added: addedCount };
                }
            }

            return { success: false, message: 'Oyuncular çözümlenemedi.' };
        } catch (e) {
            console.error('[Smart-TM] Watchlist sync error:', e);
            return { success: false, error: e.message };
        }
    }

    /**
     * Profil ve bildirim açılır menülerinden eklenti bildirimlerini çeker.
     */
    function scrapeHeaderNotifications() {
        if (typeof chrome === 'undefined' || !chrome.runtime?.id) return;

        try {
            // Find all h3, h4, span, div that say "BİLDİRİMLER" or "BILDIRIMLER"
            const headers = Array.from(document.querySelectorAll('h3, h4, span, div, p')).filter(el => {
                const txt = el.textContent.trim().toUpperCase();
                return txt === 'BİLDİRİMLER' || txt === 'BILDIRIMLER';
            });
            ...
            headers.forEach(header => {
                ...
                const links = Array.from(parent.querySelectorAll('a')).filter(a => {
                    ...
                    return text !== 'ayarlar' && !a.querySelector('svg') && text.length > 3;
                });

                links.forEach(async (link) => {
                    const text = link.textContent.trim();
                    if (!text || text.includes('Yeni bir bildirim yok')) return;
                    ...
                    if (typeof StorageManager !== 'undefined' && typeof StorageManager.addNotification === 'function') {
                        await StorageManager.addNotification(notification);
                    }
                });
            });
        } catch (e) {
            console.error('[Smart-TM] Notifications scraper error:', e);
        }
    }
    ...
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
        switch (request.action) {
            ...
            case 'syncWatchlist':
                if (settings.watchlistUrl) {
                    syncWatchlistFromUrl(settings.watchlistUrl).then(result => {
                        sendResponse(result);
                    }).catch(e => {
                        sendResponse({ success: false, error: e.message });
                    });
                } else {
                    sendResponse({ success: false, message: 'Eşitleme adresi bulunamadı. Lütfen Transfermarkt hesabınız açıkken sayfayı yenileyin.' });
                }
