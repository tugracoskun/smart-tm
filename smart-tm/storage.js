...
const StorageManager = {
   ...
   * @returns {Promise<void>}
   */
  async saveNote(transferId, noteText, playerName = null, imageUrl = null) {
    const notes = await this.getNotes();
    ...
    notes[transferId] = {
      ...
      imageUrl: imageUrl || existing.imageUrl,       // Keep existing if not provided
  ...
  /**
   * Get parsed notifications
   * @returns {Promise<Array>}
   */
  async getNotifications() {
    const list = await this.get(this.KEYS.NOTIFICATIONS);
    return list || [];
  },

  /**
   * Add a new parsed notification
   * @param {Object} notification - Notification item (text, link, timestamp, etc.)
   * @returns {Promise<void>}
   */
  async addNotification(notification) {
    ...
    const exists = list.some(item => item.text === notification.text);
    if (!exists) {
      if (!notification.timestamp) {
        notification.timestamp = new Date().toISOString();
      }
      list.unshift(notification); // Newest at the top
      
      // Limit to last 50 notifications
      if (list.length > 50) {
        list.pop();
      }
      await this.set(this.KEYS.NOTIFICATIONS, list);
    }
  },
  ...
  async clearNotifications() {
    await this.set(this.KEYS.NOTIFICATIONS, []);
  },
